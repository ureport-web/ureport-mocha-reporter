...
export class UReportApiClient {
  ...
  async createBuild(payload: UReportBuildPayload): Promise<UReportBuildResponse> {
    ...
    if (!response.ok) {
      throw new Error(
        `[ureport-mocha-reporter] Failed to create build: ${response.status} ${response.statusText}`
      );
    }
  ...
  async submitTests(tests: UReportTestPayload[]): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error(
        `[ureport-mocha-reporter] Failed to submit tests: ${response.status} ${response.statusText}`
      );
    }
  }
  ...
  async saveTestRelation(relation: UReportTestRelationPayload): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error(
        `[ureport-mocha-reporter] Failed to save test relation: ${response.status} ${response.statusText}`
      );
    }
  }
  ...
  async finalizeBuild(buildId: string): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error(
        `[ureport-mocha-reporter] Failed to finalize build: ${response.status} ${response.statusText}`
      );
    }
  }
