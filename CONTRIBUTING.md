# Contributing to ureport-mocha-reporter
...
```bash
git clone https://github.com/your-org/ureport-mocha-reporter.git
cd ureport-mocha-reporter
npm install
npm run build
npm test
```
...
```
feat: add saveRelations option
fix: coerce string booleans from Mocha CLI options
docs: clarify Cypress file/path limitation
test: add batchSize integration scenario
```
...
- Package version (`npm list ureport-mocha-reporter`)
- Node.js version (`node --version`)
- Mocha version (`npm list mocha`) or Cypress version if applicable
